...
hist(df$rate_of_interest, main ="Histogram of interest rate in 2019", xlab = "interest rate")
boxplot(df$rate_of_interest,  main ="Boxplot of interest rate in 2019", xlab = "interest rate")
...
hist(df$property_value, main ="Histogram of property value", xlab = "Property value")

# Property vs region
ggplot(df, aes(x = Region, y= property_value, color=Region))+
  geom_boxplot()+
  ylab("Property value") + xlab("Region")+
...
ggplot(df, aes(x = total_units, y= property_value, color=total_units, group=total_units))+
  geom_boxplot()+
  ylab("Property value") + xlab("Total property unit")+
...
# Income
hist(df$income,breaks = 300, main ="Histogram of income", xlab = "income")
...
describe(df)
# Income
plot(df$loan_amount,df$income, main =" Scatter plot of Income and Loan Amount")
boxplot(df$income, main ="Box plot of Income")
quantile(df$income)
...
# Property value
plot(df$income,df$property_value, main =" Scatter plot of Property value and Income")
boxplot(df$property_value, main ="Box plot of Property value")
...
ggplot(df, aes(x=as.character(Status)))+
  ...
  ylab("Density") + xlab("Status")+ theme(legend.position="none")
  ggtitle("Share of Loan status")+
  theme(plot.title = element_text(hjust = 0.5, color = "cyan4", size = 12, 
                                  face = "bold"), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
CrossTable(df$Status)
#visualization
# plot1 shows the status of applications and their percentages.
plot1 <- ggplot(df, aes(x = factor(Status))) + geom_bar(fill = "#FF6666")+ 
...
boxplot(income ~ Gender, data = df, xlab = "Gender", ylab = "Income", main = "")

#
plot(x = df$term,y = df$loan_amount,
     xlab = "Term",
     ylab = "LoanAmount",	 
...
inc_rate <- df %>% group_by(rate_of_interest) %>% 
  summarise(income_group =  mean(rate_of_interest))

ggplot(data = inc_rate, mapping = aes(x = rate_of_interest, fill = income_group)) +
  geom_histogram(aes(y=..density..), color = "green", bins = 20) +
  labs(title = "", x = "Interest rate", y = "Frequency")

##################### Aatif 
###Histogram for Loan amount based on th purpose
ggplot(data = df, mapping = aes(loan_amount, fill=factor(loan_type))) +
  geom_histogram(alpha = 0.6, position = 'identity')

###Histogram for Rate of interest based on credit type
ggplot(data = df, mapping = aes(rate_of_interest, fill=factor(credit_type))) +
  geom_histogram(alpha = 0.6, position = 'identity')

###Boxplot for credit type and loan amount stating the status of the loan
box_plot<-ggplot(df,aes(x=credit_type,y=loan_amount,fill=Credit_Worthiness))+geom_boxplot()+
...
var_factor <- df %>% select(where(is.character)) %>% glimpse() # Extract all columns in character
...
conf_matrix <- table(Actual = test_set$Status,Pred = log.stepwise.pred.test)
conf_matrix
...
log.weighted <- glm(Status ~ loan_limit + Gender + approv_in_adv + loan_type + loan_purpose + 
                       ...
                       Region, data=training_set,weights = model_weights, family = binomial(link = logit))
...
#Model accuracy
conf_matrix2 <- table(Actual = test_set$Status,Pred = log.weighted.pred.test)
conf_matrix2
confusionMatrix(t(conf_matrix2),positive = "1")

# Model comparison
# F1 Score
F1_Score(log.stepwise.pred.test, as.factor(test_set$Status), positive = "1")
F1_Score(log.weighted.pred.test, as.factor(test_set$Status), positive = "1")
# AUC
...
set.seed(345)
tree.lossmatrix <- rpart(Status ~ ., method = "class",
                          data =  training_set,
...
tree.undersample.prune.pred.test <- predict(tree.undersample.prune, newdata = test_set,  type = "class")
tree.prob.prune.pred.test <- predict(tree.prob.prune, newdata = test_set,  type = "class")
tree.lossmatrix.prune.pred.test <- predict(tree.lossmatrix.prune, newdata = test_set,  type = "class")
tree.weight.prune.pred.test <- predict(tree.weight.prune, newdata = test_set,  type = "class")

# construct confusion matrices using the predictions and confusion matrix
confmat_undersample <- table(Actual = test_set$Status,Pred = tree.undersample.prune.pred.test)
confmat_undersample
confusionMatrix(t(confmat_undersample),positive = "1")

confmat_prob <- table(Actual = test_set$Status,Pred = tree.prob.prune.pred.test)
confmat_prob
confusionMatrix(t(confmat_prob),positive = "1")

confmat_loss_matrix <- table(Actual = test_set$Status,Pred = tree.lossmatrix.prune.pred.test)
confmat_loss_matrix
confusionMatrix(t(confmat_loss_matrix),positive = "1")

confmat_weights <- table(Actual = test_set$Status,Pred = tree.weight.prune.pred.test)
confmat_weights
...
trainm <- Matrix::sparse.model.matrix(Status ~ .-1, data = trainset)
head(trainm)
train_label <- trainset[,'Status']
train_matrix <- xgb.DMatrix(data = as.matrix(trainm),label = train_label)
testm <- Matrix::sparse.model.matrix(Status ~ .-1, data = testset)
head(testm)
...
# Feature Importance
feat_imp <- xgb.importance(colnames(train_matrix),model = xgb.full)
...
rf.full.pred.test <- predict(rf.full, newdata=test_set)
# construct confusion matrices using the predictions.
rf.full.matrix <-  table(Actual = test_set$Status, Pred = rf.full.pred.test)
...
# Evaluating model performance
rf.Oversample.pred.test = predict(rf.Oversample, newdata=test_set)
# construct confusion matrices using the predictions.
rf.Oversample.matrix <-  table(Actual = test_set$Status,rf.Oversample.pred.test)
confusionMatrix(t(rf.Oversample.matrix),positive = "1")
...
# Evaluating model performance
rf.feature.pred.test = predict(rf.feature, newdata=test_set)
# construct confusion matrices using the predictions.
rf.feature.matrix <-  table(Actual = test_set$Status,rf.feature.pred.test)
confusionMatrix(t(rf.feature.matrix),positive = "1")
...
plot(strategy.rf.full$accept_rate, strategy.rf.full$bad_rate, 
     type = "l", xlab = "Acceptance rate", 
     ylab = "Bad rate", lwd = 2, main = "Default Random Forest")
plot(strategy.rf.Oversample$accept_rate, strategy.rf.Oversample$bad_rate, 
     type = "l", xlab = "Acceptance rate", 
     ylab = "Bad rate", lwd = 2, main = "Oversampled Random Forest")
plot(strategy.rf.feature$accept_rate, strategy.rf.feature$bad_rate, 
     type = "l", xlab = "Acceptance rate", 
     ylab = "Bad rate", lwd = 2, main = "Balanced Random Forest")
# test case
...
ggplot(result_IV[1:12,], aes(x= reorder(Variable,-IV), y= IV)) + 
  ...
  ylab("Information value") + xlab("Most powerful predictors")+ 
...
df %>% group_by(credit_type, Status) %>% mutate (Count = n()) %>% 
ggplot(.,aes(x= credit_type, y= Count, fill=as.character(Status))) + 
  geom_bar(position="dodge", stat="identity") +
  ylab("Count") + xlab("Credit type")+ 
  ggtitle("Default possibility by Credit type")+
  theme(plot.title = element_text(hjust = 0.5, color = "cyan4", size = 12, 
                                  face = "bold"))

# Analyse Rate of Interest
df %>% group_by(Status) %>% summarize (Mean_Interest = mean(rate_of_interest,na.rm = T),
...
ggplot(df, aes(y= property_value,x=as.character(Status), colour = as.character(Status))) + 
  geom_jitter()+ 
  geom_boxplot(alpha=0.6, size= 1) + 
  ylab("Property value") + xlab("Status type")+ 
...
df %>% group_by(lump_sum_payment, Status) %>% mutate (Count = n()) %>% 
  ggplot(.,aes(x= lump_sum_payment, y= Count, fill=as.character(Status))) + 
  geom_bar(position="dodge", stat="identity") +
  ylab("Count") + xlab("Lump sum payment")+ 
...
df %>% group_by(business_or_commercial, Status) %>% mutate (Count = n()) %>% 
  ggplot(.,aes(x= business_or_commercial, y= Count, fill=as.character(Status))) + 
  geom_bar(position="dodge", stat="identity") +
  ylab("Count") + xlab("Business/ Commercial Loan")+ 
...
df %>% group_by(Gender, Status) %>% mutate (Count = n()) %>% 
...
df %>% group_by(submission_of_application, Status) %>% mutate (Count = n()) %>% 
  ggplot(.,aes(x= submission_of_application, y= Count, fill=as.character(Status))) + 
  geom_bar(position="dodge", stat="identity") +
  ylab("Count") + xlab("Type of Submission")+ 
